...
class FetchUpcomingDraftDetailServices: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchUpcomingDraftDetailData = "Upcoming/UpcomingDraftDetail/val"
    }
    
    // MARK: - Login Variables
    var draftDetailData : NSDictionary?
    
    func fetchUpcomingDraftDetailData(userId : Int, playId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchUpcomingDraftDetailData.rawValue
        let params = ["userId":userId,"playId":playId] as [String : Any]
        FetchUpcomingDraftDetailServices.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchUpcomingDraftDetailServices()
            if let data = response as? NSDictionary {
                print(data)
                result.draftDetailData = data
