...
class AddTransactionByIdService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case addTransactionById = "VirtualTransaction/AddAmountById/val"
    }
    
    // MARK: - Login Variables
    var transactionDetailData : NSDictionary?
    
    func addTransactionByIdData(userId: Int,amount: String, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.addTransactionById.rawValue
        let params = ["UserId":userId, "Amount":Int(amount) ?? "0","Description":"Deposited in Wallet"] as [String : Any]
        AddTransactionByIdService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = AddTransactionByIdService()
            if let data = response as? NSDictionary {
                print(data)
                result.transactionDetailData = data
