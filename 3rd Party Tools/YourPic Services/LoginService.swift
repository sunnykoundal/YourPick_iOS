...
class LoginService: NSObject {
    ...
    enum WebServiceNames: String {
        ...
        case UserLogin = "AccessTokenMobileReponse/val"
    }

    // MARK: - Login Variables
    var loginData : NSDictionary?
    
    func userLogin(code: String, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.UserLogin.rawValue
     
        var deviceUUID: String = ""
        
        if let identifierForVendor = UIDevice.current.identifierForVendor {
            deviceUUID = identifierForVendor.uuidString
        }
        
        let params = ["Code": code,"TokenId":deviceUUID, "DeviceType":"ios"]
        
        LoginService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
