...
class FetchLobbiesService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/Lobby/"
        case FetchLobbies = "FetchLobbies/val"
    }
    
    // MARK: - Login Variables
    var lobbiesData : NSDictionary?
    
    func fetchLobbies(sportId: String,userId : Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.FetchLobbies.rawValue
        let params = ["intValue": sportId,"userId":userId] as [String : Any]
        FetchLobbiesService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchLobbiesService()
            if let data = response as? NSDictionary {
                print(data)
                result.lobbiesData = data
