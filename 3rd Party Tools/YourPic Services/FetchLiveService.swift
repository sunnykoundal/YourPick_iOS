...
class FetchLiveService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchLiveData = "Live/FetchLiveMatches/val"
    }
    
    // MARK: - Login Variables
    var liveData : NSDictionary?
    
    func fetchLiveData(userId: Int,createdDate: String,SportsId:Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchLiveData.rawValue
        let params = ["userId":userId,"DateValue":createdDate,"intValue":SportsId] as [String : Any]
        FetchLiveService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchLiveService()
