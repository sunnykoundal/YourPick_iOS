...
class FetchResultsService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchResults = "Result/FetchDraftResult/val"
    }
    
    // MARK: - Login Variables
    var resultsData : NSDictionary?
    
    func fetchResultsData(userId: Int,createdDate: String,SportsId:Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchResults.rawValue
        let params = ["userId":userId,"DateValue":createdDate,"intValue":SportsId] as [String : Any]
        FetchResultsService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchResultsService()
            if let data = response as? NSDictionary {
                print(data)
                result.resultsData = data
