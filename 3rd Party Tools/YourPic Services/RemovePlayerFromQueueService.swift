...
class RemovePlayerFromQueueService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case removePlayerFromQueue = "MyDraft/RemovePlayerFromQueue/val"
    }
    
    // MARK: - Login Variables
    var playerData : NSDictionary?
    
    func removePlayerFromQueue(userId: Int,playId: Int64,playerId: Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.removePlayerFromQueue.rawValue
        let params = ["userId":userId,"playId":playId,"playerId":playerId] as [String : Any]
        RemovePlayerFromQueueService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = RemovePlayerFromQueueService()
