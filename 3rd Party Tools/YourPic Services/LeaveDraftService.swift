...
class LeaveDraftService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case leaveDraft = "MyDraft/LeaveDraft/val"
    }
    
    // MARK: - Login Variables
    var leftDraftData : NSDictionary?
    
    func leaveDraft(userId : Int, playId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.leaveDraft.rawValue
        let params = ["playId":playId, "userId":userId] as [String : Any]
        LeaveDraftService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = LeaveDraftService()
            if let data = response as? NSDictionary {
                print(data)
                result.leftDraftData = data
