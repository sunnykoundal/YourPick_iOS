...
class FetchDraftResultService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchDraftResults = "Result/DraftResultS/val"
    }
    
    // MARK: - Login Variables
    var draftResultsData : NSDictionary?
    
    func fetchDraftResultsData(userId: Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchDraftResults.rawValue
        let params = ["userId":userId] as [String : Any]
        FetchDraftResultService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchDraftResultService()
            if let data = response as? NSDictionary {
                print(data)
                result.draftResultsData = data
