...
class MoveQueuePLayersService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case moveQueuePLayers = "MyDraft/MoveQueuePlayers/val"
    }
    
    // MARK: - Login Variables
    var queuePlayerData : NSDictionary?
    
    func moveQueuePLayers(userId: Int,playId: Int64,playerRowPOsition: NSArray, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.moveQueuePLayers.rawValue
        let params = ["userId":userId,"playId":playId,"IntArray":playerRowPOsition] as [String : Any]
        AddPlayerToQueueService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = MoveQueuePLayersService()
            if let data = response as? NSDictionary {
                print(data)
                result.queuePlayerData = data
