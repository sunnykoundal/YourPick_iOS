...
class FetchUserPlayerPicksService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchUserPlayerPicks = "MyDraft/UserPlayerPicks/val"
    }
    
    // MARK: - Login Variables
    var userPlayerPicksData : NSDictionary?
    
    func fetchfetchUserPlayerPicksData(playId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchUserPlayerPicks.rawValue
        let params = ["playId":playId] as [String : Any]
        FetchUserPlayerPicksService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            ...
            let result = FetchUserPlayerPicksService()
            if let data = response as? NSDictionary {
                print(data)
                result.userPlayerPicksData = data
