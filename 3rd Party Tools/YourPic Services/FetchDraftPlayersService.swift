...
class FetchDraftPlayersService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchDraftPlayers = "MyDraft/FetchDraftPlayers/val"
    }
    
    // MARK: - Login Variables
    var draftPlayersData : NSDictionary?
    
    func fetchDraftPlayersData(playId : Int64,sportsId : Int64,userId: Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchDraftPlayers.rawValue
        let params = ["intValue": sportsId, "userId": userId, "playId":playId] as [String : Any]
        FetchUserPlayerPicksService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchDraftPlayersService()
            if let data = response as? NSDictionary {
                print(data)
                result.draftPlayersData = data
