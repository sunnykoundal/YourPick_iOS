...
class FetchBalanceService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchBalance = "Balance/FetchBalance/val"
    }
    
    // MARK: - Login Variables
    var playerDetailsData : NSDictionary?
    
    func fetchBalanceData(userId: Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchBalance.rawValue
        let params = ["UserId":userId] as [String : Any]
        FetchBalanceService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchBalanceService()
