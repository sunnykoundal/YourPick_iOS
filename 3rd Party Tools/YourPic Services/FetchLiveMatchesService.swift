...
class FetchLiveMatchesService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchLiveMatches = "Live/LiveMatches/val"
    }
    
    // MARK: - Login Variables
    var liveMatchesData : NSDictionary?
    
    func fetchLiveMatchesData(userId: Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchLiveMatches.rawValue
        let params = ["userId":userId] as [String : Any]
        FetchLiveMatchesService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchLiveMatchesService()
            if let data = response as? NSDictionary {
                print(data)
                result.liveMatchesData = data
