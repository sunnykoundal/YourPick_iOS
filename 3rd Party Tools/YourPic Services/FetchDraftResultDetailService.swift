...
class FetchDraftResultDetailService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchDraftResultDetails = "Result/DraftResultDetail/val"
    }
    
    // MARK: - Login Variables
    var draftResultDetailData : NSDictionary?
    
    func fetchDraftResultDetailsData(userId: Int,playId: Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchDraftResultDetails.rawValue
        let params = ["userId":userId,"playId":playId] as [String : Any]
        FetchDraftResultDetailService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchDraftResultDetailService()
            if let data = response as? NSDictionary {
                print(data)
                result.draftResultDetailData = data
