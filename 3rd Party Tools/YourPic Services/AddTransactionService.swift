...
class AddTransactionService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case addTransaction = "PayPal/AddTransaction/val"
    }
    
    // MARK: - Login Variables
    var playerDetailsData : NSDictionary?
    
    func addTransactionData(transactionId : String,userId: Int,amount: String, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.addTransaction.rawValue
        let params = ["TransactionId":transactionId, "UserId":userId, "Amount":Int(amount) ?? "0"] as [String : Any]
        AddTransactionService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = AddTransactionService()
