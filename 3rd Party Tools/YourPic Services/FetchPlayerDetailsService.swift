...
class FetchPlayerDetailsService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchPlayerDetails = "MyDraft/FetchPlayerDetails/val"
        case autoPickOnOff = "MyDraft/SetAutoPick/val"
    }
    
    // MARK: - Login Variables
    var playerDetailsData : NSDictionary?
    
    func fetchPlayerDetailsData(playId : Int64 ,sportsId: Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchPlayerDetails.rawValue
        let params = ["playerId":playId, "intValue":sportsId] as [String : Any]
        FetchPlayerDetailsService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchPlayerDetailsService()
            if let data = response as? NSDictionary {
                print(data)
                result.playerDetailsData = data
    ...
    func fetchAutoPickData(playId : Int64 ,userID: Int, boolValue: Bool, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.autoPickOnOff.rawValue
        let params = ["playId":playId, "userId":userID, "boolValue": boolValue] as [String : Any]
        FetchPlayerDetailsService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchPlayerDetailsService()
            if let data = response as? NSDictionary {
