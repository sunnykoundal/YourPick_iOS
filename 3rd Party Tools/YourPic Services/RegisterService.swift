...
class RegisterService: NSObject {
    ...
    enum WebServiceNames: String {
        ...
        case RegisterUser = "Register/val"
    }
    
    // MARK: - Login Variables
    var registerData : NSDictionary?
    
    func registerUser(userName: String,password : String,confirmPassword : String,email : String, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.RegisterUser.rawValue
        let params = ["UserName": userName,"Password":password, "ConfirmPassword":confirmPassword,"Email":email]
        RegisterService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
