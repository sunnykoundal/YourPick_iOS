...
class SignOutService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case logout = "Account/RemoveToken/val"
    }
    
    // MARK: - Login Variables
    var logoutUserData : NSDictionary?
    
    func logoutUser(userId: Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.logout.rawValue
        let params = ["UserId":userId] as [String : Any]
        SignOutService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = SignOutService()
            if let data = response as? NSDictionary {
                print(data)
                result.logoutUserData = data
