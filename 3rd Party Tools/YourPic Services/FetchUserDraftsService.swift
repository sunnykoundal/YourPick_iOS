...
class FetchUserDraftsService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchUserDrafts = "MyDraft/FetchUserDrafts/val"
    }
    
    // MARK: - Login Variables
    var userDraftsData : NSDictionary?
    
    func fetchUserDrafts(userId : Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchUserDrafts.rawValue
        let params = ["userId":userId] as [String : Any]
        FetchUserDraftsService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchUserDraftsService()
            if let data = response as? NSDictionary {
                print(data)
                result.userDraftsData = data
