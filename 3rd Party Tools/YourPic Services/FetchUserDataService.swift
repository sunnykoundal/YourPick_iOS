...
class FetchUserDataService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchUserData = "Account/MyProfile/val"
    }
    
    // MARK: - Login Variables
    var userData : NSDictionary?
    
    func fetchUserData(userId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchUserData.rawValue
        let params = ["userId":userId] as [String : Any]
        FetchUserDataService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchUserDataService()
