//
//  FetchUnselectedPlayerService.swift
//  YourPick
//
//  Created by Krishna-Mac on 15/10/18.
...
class FetchUnselectedPlayerService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchUnselectedPlayerDetail = "Upcoming/FetchUnSelectedPlayers/val"
    }
    
    // MARK: - Login Variables
    var unselectedPlayerDetailData : NSDictionary?
    
    func fetchUnselectedPlayerDetailData(userId: Int,playId: Int64, intValue: Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchUnselectedPlayerDetail.rawValue
        let params = ["userId":userId,"playId":playId,"intValue":intValue] as [String : Any]
        FetchUnselectedPlayerService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchUnselectedPlayerService()
            if let data = response as? NSDictionary {
                print(data)
                result.unselectedPlayerDetailData = data
