...
class AddPlayerToQueueService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case addPlayerToQueue = "MyDraft/AddPlayerToQueue/val"
    }
    
    // MARK: - Login Variables
    var playerData : NSDictionary?
    
    func addPlayerToQueue(userId: Int,playId: Int64,playerId: Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.addPlayerToQueue.rawValue
        ...
        AddPlayerToQueueService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            ...
            let result = AddPlayerToQueueService()
