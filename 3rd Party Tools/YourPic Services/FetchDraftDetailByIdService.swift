...
class FetchDraftDetailByIdService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchDraftDetailById = "MyDraft/FetchDraftById/val"
    }
        
    // MARK: - Login Variables
    var draftdetailData : NSDictionary?
    
    func fetchDraftById(draftId: Int,userId : Int, playId : Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchDraftDetailById.rawValue
        let params = ["draftId": draftId,"playId":playId, "userId":userId] as [String : Any]
        FetchDraftByIdService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchDraftByIdService()
