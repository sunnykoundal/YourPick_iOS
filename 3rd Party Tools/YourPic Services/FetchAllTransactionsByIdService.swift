...
class FetchAllTransactionsByIdService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchAllTransactionsByIdData = "VirtualTransaction/FetchAllTransactionsById/val"
    }
    
    // MARK: - Login Variables
    var transactionData : NSDictionary?
    
    func fetchAllTransactionsByIdData(userId : Int, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchAllTransactionsByIdData.rawValue
        let params = ["userId":userId,"intValue":2] as [String : Any]
        FetchAllTransactionsByIdService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = FetchAllTransactionsByIdService()
