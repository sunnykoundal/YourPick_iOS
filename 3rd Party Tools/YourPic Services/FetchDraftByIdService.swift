...
class FetchDraftByIdService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/"
        case fetchDraftById = "MyDraft/FetchDraftById/val"
    }
    
    // MARK: - Login Variables
    var draftData : NSDictionary?
    
    func fetchDraftById(draftId: Int,userId : Int, playId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.fetchDraftById.rawValue
