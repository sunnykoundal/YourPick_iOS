...
class EnterDraftService: NSObject {
    
    enum WebServiceNames: String {
        case baseUrl = "http://200.124.153.227/api/Lobby/"
        case enterDraft = "EnterDraft/val"
    }
    
    // MARK: - Login Variables
    var playData : NSDictionary?
    
    func enterDraft(draftId: Int,userId : Int, playId : Int64, completion : @escaping (_ response : AnyObject?, _ message: String?, _ success : Bool)-> ()) {
        let url = WebServiceNames.baseUrl.rawValue + WebServiceNames.enterDraft.rawValue
        let params = ["draftId": draftId,"playId":playId, "userId":userId] as [String : Any]
        EnterDraftService.postWebService(urlString: url, params: params as [String : AnyObject]) { (response, message, status) in
            print(response ?? "Error")
            let result = EnterDraftService()
            if let data = response as? NSDictionary {
                print(data)
                result.playData = data
